...
public final class PushBridge: NSObject, PKPushRegistryDelegate {
    ...
    private func answered() {
        ...
        ck.onActivate = {
            do {
                try e.start(callId: "ring-" + String(UUID().uuidString.lowercased().prefix(12)))
                e.nudge("[Call connected: YOU rang him and he just answered — about: "
                        + text + " — say why you called in one sentence, then talk.]")
            } catch {
